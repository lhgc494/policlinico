...
def reporte_proximos_vencer(request):
    ...
    # Filtrar usando propiedades (en memoria) - ¡CORREGIDO!
    medicamentos_proximos = [
        m for m in medicamentos_activos 
        if m.proximo_vencer  # Esta es una propiedad, no campo de BD
    ]
    
    # Ordenar por fecha de vencimiento
    # Usar date.max para medicamentos sin fecha
    from datetime import date
    medicamentos_proximos.sort(key=lambda x: x.fecha_vencimiento or date.max)

    context = {
        'medicamentos': medicamentos_proximos,
        'titulo': 'Medicamentos Próximos a Vencer',
        'total': len(medicamentos_proximos)
    }
...
def detalle_recetas_consulta(request, consulta_id):
    """Muestra detalle de recetas - STOCK como límite"""
    from consultas.models import Consulta
    
    consulta = get_object_or_404(Consulta, id=consulta_id)
    recetas = consulta.recetas.all().order_by('medicamento')
    ...
    for receta in recetas:
        try:
            ...
            if medicamento_qs:
                ...
                # 3. Cantidad sugerida: todo el stock o 0 si ya está atendida
                if receta.estado != 'ATENDIDA' and stock_disponible > 0:
...
def procesar_venta_directa(request):
    print("="*60)
    print("DEBUG: procesar_venta_directa INICIADA")
    print(f"User: {request.user}")
    carrito = request.session.get("carrito_venta_directa", [])
    print(f"Carrito items: {len(carrito)}")
    print(f"Carrito: {carrito}")
    if not carrito:
        print("DEBUG: Carrito vacío!")
    """Procesa una venta directa desde el carrito"""
    ...
    try:
        ...
        # CREAR VENTA EN UNA TRANSACCIÓN (para evitar inconsistencia)
        from django.db import transaction
        ...
        try:
            with transaction.atomic():
                ...
                for detalle_data in detalles_a_crear:
                    ...
                    # Actualizar stock
                    medicamento = detalle_data['medicamento']
                    medicamento.stock_actual -= detalle_data['cantidad']
                    medicamento.save()
                ...
                # LIMPIAR CARRITO INMEDIATAMENTE después de éxito
    ...
    if tipo == 'consulta':
        ...
        if request.method == 'POST':
            try:
                            ...
                            try:
                                ...
                                if medicamento and medicamento.stock_actual >= cantidad:
                                    ...
                                    venta = Venta.objects.create(
                                        ...
                                        descuento=descuento,
                                        metodo_pago=metodo_pago,
                                        observaciones=f'Venta desde receta - Consulta #{consulta.id}',
                                        usuario=request.user
                                    )
                ...
                # Usar el ticket de venta
                if recetas_atendidas:
        ...
        # Lógica para venta directa - REDIRIGIR al ticket
...
def buscar_medicamentos_ajax(request):
    """úsqueda AJAX para venta directa"""
