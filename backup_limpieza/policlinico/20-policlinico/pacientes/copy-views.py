...
def inicio(request):
    ...
    if request.user.is_authenticated:
        ...
        # 3. Si está en grupo "doctores" pero no tiene doctor asociado
        if request.user.groups.filter(name='doctores').exists():
            return redirect('doctor_consultas')
        
        # 4. Si está en grupo "recepcion" o es admin → recepción
        if request.user.groups.filter(name='recepcion').exists() or request.user.is_superuser or request.user.is_staff:
            return redirect('lista_pacientes')
        
        # 5. Para otros usuarios sin grupo → página de inicio
